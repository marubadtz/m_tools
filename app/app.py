...
app._favicon = ('favicon64nobg.png')

# Functions
# Get ready to format url
def get_universal_url(url):
    for l in langs + ['zh-cn']:
        url = url.replace('/'+l+'/', '/')
        url = url.replace('/eu-'+l+'/', '/').replace('/na-'+l+'/', '/').replace('/asia-'+l+'/', '/')
    for d in domains:
        url = url.replace('.'+d+'/', '.')
    url = url.replace('armory.','').replace('promo.','').replace('news_ingame/','')
...
domain_lang_map = {'eu':'eu', 'com':'na', 'asia':'asia', 'cn':'cn'}
...
input_name = dbc.Input(id='input_name', placeholder="MC3283_WGC_v1305_CLLB_Popeye_MAU_PY")
check_armory = dcc.Checklist(['Armory'], [], id='check_armory', labelStyle={"display": "none", "align-items": "center"})
check_promo = dcc.Checklist(['promo.'], [], id='check_promo', labelStyle={"display": "flex", "align-items": "center"})
...
input_url = dbc.Input(id='input_url', placeholder="https://worldofwarships.eu/news/sales-and-events/popeye-and-bluto-are-back-in-wows/")
...
html.Div([
    ...
    dbc.Row([dbc.Col([input_name], width=6), 
             dbc.Col([check_promo], width=1), dbc.Col([check_lang], width=1), dbc.Col([check_ingame], width=2), dbc.Col([check_armory], width=0)]),   
...
# This is new branch
@callback(Output('check_armory', 'value'), Output('check_promo', 'value'), Output('check_lang', 'value'), Output('check_ingame', 'value'), 
          Input('input_url', 'value'), prevent_initial_call=True)
def check_url_params(url):
    if url_is_valid(url):
        armory = ['Armory'] if 'armory.' in url else ['']
        promo = ['promo.'] if 'promo.' in url else ['']
        lang = ['/lang/'] if any(['/'+l+'/' in url for l in langs + ['zh-cn']]) \
                else ['/lang/'] if any(['/'+c+'-'+l+'/' in url \
                                        for l in langs + ['zh-cn'] for c in ['eu', 'na', 'asia']]) else ['']
        news_ingame = ['/news_ingame/'] if '/news_ingame/' in url else ['']
        return armory, promo, lang, news_ingame
    else:
        return [], [], [], []

# Update json example on parametars changes
@callback(Output('json_example', 'value'), Output('url_note', 'children'),
          Input('input_url', 'value'), Input('input_name', 'value'), Input('check_armory', 'value'), Input('check_promo', 'value'), 
          Input('check_lang', 'value'), Input('check_ingame', 'value'), 
          State('input_name', 'placeholder'), State('input_url', 'placeholder'),
          prevent_initial_call=True)
def update_json_example(val_url, val_name, val_armory, val_promo, val_lang, val_news_ingame, name_place, url_place):
    armory = 'armory.' if 'Armory' in val_armory  else ''
    promo = 'promo.' if 'promo.' in val_promo  else ''
    ...
    if promo == 'promo.':
        lang = '' if '/lang/' not in val_lang else country_domain_map['AD']+'-en/' if '.cn/' not in url else 'zh-cn/'
    else:
        lang = '' if '/lang/' not in val_lang else 'es/' if '.cn/' not in url else 'zh-cn/'
    domain = 'cn'  if '.cn/' in url  else country_domain_map['AD']
    country = 'O1'  if '.cn/' in url  else 'AD'
    if url_is_valid(url):
        if promo == 'promo.':
            url_univ = get_universal_url(url) % (promo, '')
            url_univ = url_univ.format(domain + '/', '', lang)
        else:
        ...
        url_univ = url_univ.format(domain + '/', lang, '')
        url_note = 'wrong url'
    return text_temp % (code, name, url_univ, country), url_note

# Download json on button click
@callback(Output('download_json', 'data'), 
          Input('btn_download', 'n_clicks'), State('input_url', 'value'), State('input_name', 'value'), State('check_armory', 'value'), State('check_promo', 'value'), 
          State('check_lang', 'value'), State('check_ingame', 'value'), State('url_note', 'children'), prevent_initial_call=True)
def download_json(n_clicks, url, val_name, val_armory, val_promo, val_lang, val_news_ingame, url_note):
    if n_clicks > 0 and url is not None and val_name is not None and url_note == '':
        ...
        if promo == 'promo.':
            url_univ = get_universal_url(url) % (promo, '')
            country_lang_map_used = {'O1':'zh-cn'} if '.cn/' in url else country_lang_map_promo
        else:
            url_univ = get_universal_url(url) % (armory, news_ingame)
            country_lang_map_used = {'O1':'zh-cn'} if '.cn/' in url else country_lang_map
        code = val_name.lower().replace(' ', '-').replace('_', '-')
        template_data= {"lpsets": [{"note": None, "code": code, "name": val_name, "urls": []}]}
        country_domain_map_used = {'O1':'cn'} if '.cn/' in url else country_domain_map
        # Create json tempalte
        for country, lang in country_lang_map_used.items():
            if promo == 'promo.':
                lang = domain_lang_map[country_domain_map_used[country]] + '-' + lang + '/' if '/lang/' in val_lang else ''
                template_data['lpsets'][0]['urls'].append({'url':url_univ.format(country_domain_map_used[country] + '/', '', lang), 'country':country})
            else:
